...
static const uint32_t pair_colors[] = {
    0xF87171, 0x34D399, 0x60A5FA, 0xFBBF24,
    0xA78BFA, 0xF472B6, 0x2DD4BF, 0xFB923C
};

static const char* pair_chars[] = {
    "A","B","C","D","E","F","G","H"
};
...
static void pairs_render(int id, int x, int y, int w, int h, int vx, int vy) {
    ...
    for (int i = 0; i < CARDS; i++) {
        ...
        if (matched[i] || revealed[i]) {
            uint32_t col = matched[i] ? pair_colors[cards[i]] : gfx_lighten(pair_colors[cards[i]], 30);
            gfx_fill_rect(cx, cy, CARD, CARD, col);
            gfx_draw_rect_outline(cx, cy, CARD, CARD, 1, 0x444466);
            gfx_draw_string_transparent(cx + 18, cy + 16, pair_chars[cards[i]], 0xFFFFFF);
        } else {
            gfx_fill_rect(cx, cy, CARD, CARD, 0x1E1E3A);
            gfx_draw_rect_outline(cx, cy, CARD, CARD, 1, 0x333355);
            gfx_draw_string_transparent(cx + 18, cy + 16, "?", 0x555577);
        }
    }
