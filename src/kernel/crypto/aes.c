...
void aes128_init(aes128_ctx* ctx, const uint8_t* key) {
    ...
    for (int i = 4; i < 44; ++i) {
        ...
        if (i % 4 == 0) {
            temp = (temp << 8) | (temp >> 24); // rot_word
            temp = sub_word(temp) ^ (Rcon[i/4] << 24);
        }
...
static void mix_columns(uint8_t* state) {
    ...
    for (int i = 0; i < 4; ++i) {
        uint8_t* c = state + i*4;
        tmp[i*4]   = multiply(0x02, c[0]) ^ multiply(0x03, c[1]) ^ c[2] ^ c[3];
        tmp[i*4+1] = c[0] ^ multiply(0x02, c[1]) ^ multiply(0x03, c[2]) ^ c[3];
        tmp[i*4+2] = c[0] ^ c[1] ^ multiply(0x02, c[2]) ^ multiply(0x03, c[3]);
        tmp[i*4+3] = multiply(0x03, c[0]) ^ c[1] ^ c[2] ^ multiply(0x02, c[3]);
    }
    memcpy(state, tmp, 16);
}
...
static void inv_mix_columns(uint8_t* state) {
    ...
    for (int i = 0; i < 4; ++i) {
        uint8_t* c = state + i*4;
        tmp[i*4]   = multiply(0x0e, c[0]) ^ multiply(0x0b, c[1]) ^ multiply(0x0d, c[2]) ^ multiply(0x09, c[3]);
        tmp[i*4+1] = multiply(0x09, c[0]) ^ multiply(0x0e, c[1]) ^ multiply(0x0b, c[2]) ^ multiply(0x0d, c[3]);
        tmp[i*4+2] = multiply(0x0d, c[0]) ^ multiply(0x09, c[1]) ^ multiply(0x0e, c[2]) ^ multiply(0x0b, c[3]);
        tmp[i*4+3] = multiply(0x0b, c[0]) ^ multiply(0x0d, c[1]) ^ multiply(0x09, c[2]) ^ multiply(0x0e, c[3]);
    }
    memcpy(state, tmp, 16);
}
...
void aes128_cbc_encrypt(const aes128_ctx* ctx, uint8_t* iv, const uint8_t* in, uint8_t* out, size_t length) {
    ...
    for (size_t i = 0; i < length; i += 16) {
        ...
        aes128_encrypt_block(ctx, block, out + i);
        memcpy(iv, out + i, 16);
    }
}
...
void aes128_cbc_decrypt(const aes128_ctx* ctx, uint8_t* iv, const uint8_t* in, uint8_t* out, size_t length) {
    uint8_t next_iv[16];
    for (size_t i = 0; i < length; i += 16) {
        memcpy(next_iv, in + i, 16);
        aes128_decrypt_block(ctx, in + i, out + i);
        for (int j = 0; j < 16; ++j) out[i + j] ^= iv[j];
