...
void tls_prf_sha256(const uint8_t* secret, size_t secret_len, 
    ...
    memcpy(seed_buf + label_len, seed, seed_len);
    size_t seed_buf_len = label_len + seed_len;

    uint8_t a[32];
    hmac_sha256(secret, secret_len, seed_buf, seed_buf_len, a);

    size_t generated = 0;
    while (generated < out_len) {
        uint8_t iter_buf[160];
        memcpy(iter_buf, a, 32);
        memcpy(iter_buf + 32, seed_buf, seed_buf_len);
        
        uint8_t hash[32];
        hmac_sha256(secret, secret_len, iter_buf, 32 + seed_buf_len, hash);
