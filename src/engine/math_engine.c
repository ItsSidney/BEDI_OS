...
float  vec2_len(vec2_t a) { return (float)sqrt((double)(a.x*a.x + a.y*a.y)); }
...
float  vec3_len(vec3_t a) { return (float)sqrt((double)(a.x*a.x + a.y*a.y + a.z*a.z)); }
...
mat4_t mat4_perspective(float fov_rad, float aspect, float near, float far) {
    mat4_t m = {{{0}}};
    float f = 1.0f / (float)tan(fov_rad * 0.5);
    m.m[0][0] = f / aspect;
    m.m[1][1] = f;
...
quat_t quat_rotate(float rad, float ax, float ay, float az) {
    float len = (float)sqrt((double)(ax*ax+ay*ay+az*az));
    if (len==0) return quat_identity();
    float s = (float)sin(rad*0.5f);
    return quat(ax/len*s, ay/len*s, az/len*s, (float)cos(rad*0.5f));
}
...
static etok_t etok_cur;
static double etok_val;
...
static void next_tok(void) {
    ...
    if (c == 0) { etok_cur = TOK_EOF; return; }
    if (c == '+') { expr_pos++; etok_cur = TOK_PLUS; return; }
    if (c == '-') { expr_pos++; etok_cur = TOK_MINUS; return; }
    if (c == '*') { expr_pos++; etok_cur = TOK_MUL; return; }
    if (c == '/') { expr_pos++; etok_cur = TOK_DIV; return; }
    if (c == '^') { expr_pos++; etok_cur = TOK_POW; return; }
    if (c == '(') { expr_pos++; etok_cur = TOK_LP; return; }
    if (c == ')') { expr_pos++; etok_cur = TOK_RP; return; }
    if (c == ',') { expr_pos++; etok_cur = TOK_COMMA; return; }
    if (c == 'x' || c == 'X') { expr_pos++; etok_cur = TOK_X; return; }
    if ((c >= '0' && c <= '9') || c == '.') {
    ...
    if (is_alpha(c)) {
        ...
        while (bi < 31 && is_alpha(expr_src[expr_pos]))
            buf[bi++] = expr_src[expr_pos++];
        buf[bi] = 0;
        if      (!strcmp(buf,"sin"))  etok_cur=TOK_SIN;
        else if (!strcmp(buf,"cos"))  etok_cur=TOK_COS;
        else if (!strcmp(buf,"tan"))  etok_cur=TOK_TAN;
        else if (!strcmp(buf,"sqrt")) etok_cur=TOK_SQRT;
        else if (!strcmp(buf,"abs"))  etok_cur=TOK_ABS;
        else if (!strcmp(buf,"log"))  etok_cur=TOK_LOG;
        else if (!strcmp(buf,"ln"))   etok_cur=TOK_LN;
        else if (!strcmp(buf,"exp"))  etok_cur=TOK_EXP;
        else { snprintf(expr_err,128,"Unknown: %s", buf); etok_cur=TOK_EOF; }
        return;
    }
    snprintf(expr_err,128,"Unexpected '%c'", c);
    etok_cur = TOK_EOF;
}
...
static double parse_primary(void) {
    if (etok_cur == TOK_NUM) { double v=etok_val; next_tok(); return v; }
    if (etok_cur == TOK_X) { next_tok(); return expr_var_x; }
    if (etok_cur == TOK_MINUS) { next_tok(); return -parse_primary(); }
    if (etok_cur == TOK_LP) {
        next_tok(); double v = parse_expr();
        if (etok_cur == TOK_RP) next_tok();
    ...
    if (ft >= TOK_SIN && ft <= TOK_EXP) {
        ...
        switch (ft) {
            ...
            case TOK_SQRT: return arg<0 ? (snprintf(expr_err,128,"sqrt(neg)"),0.0) : sqrt(arg);
            case TOK_ABS:  return fabs(arg);
            case TOK_LOG:  return arg<=0 ? (snprintf(expr_err,128,"log(<=0)"),0.0) : log10(arg);
            case TOK_LN:   return arg<=0 ? (snprintf(expr_err,128,"ln(<=0)"),0.0) : log(arg);
            case TOK_EXP:  return exp(arg);
            default: return 0;
        }
    }
    snprintf(expr_err,128,"Syntax error");
    return 0;
}

static double parse_pow(void) {
    double v = parse_primary();
    if (etok_cur == TOK_POW) { next_tok(); v = pow(v, parse_pow()); }
    return v;
}

static double parse_mul(void) {
    double v = parse_pow();
    while (etok_cur == TOK_MUL || etok_cur == TOK_DIV) {
        etok_t op = etok_cur; next_tok();
        double r = parse_pow();
        if (op == TOK_MUL) v *= r; else v /= r;
    }
    return v;
}
...
static double parse_expr(void) {
    double v = parse_mul();
    while (etok_cur == TOK_PLUS || etok_cur == TOK_MINUS) {
        etok_t op = etok_cur; next_tok();
        double r = parse_mul();
